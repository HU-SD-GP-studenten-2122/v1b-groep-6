...
public class afmeldschermController {
    ...
    ObservableList<String> redenen = FXCollections.observableArrayList("Ziek", "Doktersbezoek", "Afspraak met instantie", "Overlijden Familielid","Overige");
    ObservableList<Klas> alleKlassen = FXCollections.observableArrayList(School.getAlleKlassen());

    public void initialize() {
        try {
            afwezigheidDatumpicker.setValue(LocalDate.now());
        ...
        for (Klas k : alleKlassen) {
            for (Student s : k.getStudenten()) {
                if (s.getNaam().equals(SysteemGebruikers.gethuidigegebruikernaam())) {
                    if (afwezigheidDatumpicker.getValue() == null || afaanmeldDropdown.getValue() == null) {
